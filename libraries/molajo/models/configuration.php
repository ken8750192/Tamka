...
class MolajoModelConfiguration extends JModel
{
   /**
     ...
     * @var string
     */
    public $table = '#__molajo_configuration';
    ...
    public function getSingleConfigurationValue ($option_id)
    {
        ...
        if ($component_option === false) {
            $component_option = 'com_molajo';
        }

        /** validation query **/
        $query->select($db->namequote('option_value'), $db->namequote('option_value_literal'));
        $query->from($db->namequote('#__molajo_configuration'));
   ...
   /**
     ...
     * The component_option com_molajo is used when there is no override for that option_id set
